...
type Props = {
  type: "image" | "line" | "paragraph" | "card";
  title?: boolean;
};

const Loading = ({ type, title }: Props) => {
  switch (type) {
    case "line":
      return (
        <div role="status" className="animate-pulse w-full">
          <div className={`${title ? "h-12" : "h-3"} bg-gray-200 rounded-full dark:bg-gray-700 w-full mb-4`}></div>
        </div>
      );
    ...
    case "card":
      return (
        <div className="prompt_card">
          ...
          <div className="h-2 my-4 bg-gray-200 rounded-full dark:bg-gray-700"></div>
